# Frontend školení

## 1. PRINCIPY HTML & CSS

### HTML
 - [prezentace](prezentace/1-1_HTML.pdf)
 - [html elementy](1-1_html/html-elementy.html)
 - [cvičení návrh HTML stránky](1-1_html/ukol-html5.html)

### Principy CSS
 - [prezentace CSS kaskáda](prezentace/1-2_CSS-kaskada.pdf)
 - [kaskáda prakticky](1-2_css-principy/kaskada.html)
 - [selektory](1-2_css-principy/selektory.html)
 - [box-model](1-2_css-principy/box-model.html)
 - [vlastnost display](1-2_css-principy/display.html)
 - [prezentace CSS jednotky](prezentace/1-2_CSS-jednotky.pdf)
 - [jednotky prakticky](1-2_css-principy/jednotky.html)
 - [obrázky](1-2_css-principy/obrazky.html)
 - [background](1-2_css-principy/background.html)
 - [z-index](1-2_css-principy/z-index.html)
 - [funkce calc](1-2_css-principy/calc.html)
 - [css proměnné](1-2_css-principy/css-promenne.html)
 - [webfonty](1-2_css-principy/webfonty.html)
 - [transformace](https://developer.mozilla.org/en-US/docs/Web/CSS/transform)

### Flexbox a grid
 - [prezentace CSS flex & grid](prezentace/1-3_CSS-flex.pdf)
 - [flexbox prakticky](1-3_flex-grid/flexbox.html)
 - [vyzkoušejte flexbox](https://codepen.io/svobodalukas/pen/PyLpjN)
 - [flexbox hra prakticky](https://flexboxfroggy.com/#cs)
 - [grid prakticky](1-3_flex-grid/grid.html)
 - [grid hra](http://cssgridgarden.com/)

### SVG
 - [prezentace SVG](prezentace/1-4_SVG.pdf)
 - [SVG prakticky](1-4_svg/svg.html)
 - [SVG animace](1-4_svg/svg-animace.html)

## 2. Organizace CSS & refaktoring
### Systémy organizace kódu
 - [prezentace SASS](prezentace/2-1_Organizace-CSS.pdf)
 - [refaktoring dle OOCSS & BEM](2-1_Organizace-kodu/refaktoring-oocss.html)

### Další možnosti zápisu CSS
 - [utility CSS - Tailwind](https://tailwindcss.com/)
 - [Tailwind stack](2-2_jine-systemy/gatsbyjs-starter-tailwindplay)
 - [CSS in JS - Emotion](https://emotion.sh/docs/introduction)
 - [Emotion stack](2-2_jine-systemy/css-js-site)

### Hledání chyb
 - [prezentace refaktoring a hledání chyb](prezentace/2-3_Chyby.pdf)
 - [HTML Validátor](https://validator.w3.org/)
 - [Browserstack](https://www.browserstack.com/)
 - [Lambdatest](https://www.lambdatest.com/)
 - [refaktoring komponenty](https://codepen.io/svobodalukas/pen/oapmWw)

## 3. Responsivita a automatizace
### Principy responsivity
 - [prezentace responsivita](prezentace/3-1_Responsivita.pdf)
 - [responsivní obrázky](3-1_responsivita/obrazky.html)
 - [responsivní video](3-1_responsivita/video-iframe.html)
 - [responsivní typografie](3-1_responsivita/typografie.html)
 - [responsivní tabulky](3-1_responsivita/tabulky.html)

### Preprocesory a automatizace
 - [prezentace SASS](prezentace/3-2_SASS.pdf)
 - [SCSS prakticky, automatizace - GULP](3-2_preprocesory/gulp-stack/index.html)
 - [Webpack](3-2_preprocesory/webpack-stack/index.html)

### Bootstrap
 - [Bootstrap web](https://getbootstrap.com/)
 - [Bootstrap ukázka](3-3_bootstrap/index.html)
 - [Bootstrap jako npm](3-3_bootstrap/bootstrap-npm/)

### Trocha javascriptu pro kodéry
 - [javascript](3-4_javascript/index.html)

### Další js stacky pro vývoj
 - [Jamstack + Storybook](https://github.com/markoradak/gatsby-starter-storybook)
 - [Jamstack + Storybook code](3-5_js-stack/gatsby-starter-storybook/)
 - [Vue starter pack](https://github.com/qraimbault/vue-js-starter-scss)
 - [Vue starter code](3-5_js-stack/vue-js-starter/)
 - [AMP web](https://amp.dev/)
 - [AMP ukázka](3-5_js-stack/amp/index-amp.html)

## 4. Rychlost načítání a přístupnost

### Rychlost načítání
 - [prezentace rychlost načítání](prezentace/4-1_Rychlost-nacitani.pdf)

### Pravidla přístupného webu
 - [prezentace přístupnost](prezentace/4-2_Pristupnost.pdf)
 - [chyby v UI](prezentace/4-2_UI-faily)
 - [Bad UI test](https://userinyerface.com/)